...
const VARIANT: Record<string, string> = { workflow: 'bx bx--feat', neukunden: 'bx bx--wide' };
        ...
        <div className="bento">
          {tiles.map((tile, i) => (
            <Reveal
              ...
              delay={(i % 3) * 0.08}
            >
              {tile.id === 'workflow' ? <span className="tag">{t('tag')}</span> : null}
              <Ico id={tile.id} size={tile.id === 'workflow' ? 24 : 22} />
              <h3>{tile.title}</h3>
              <p>{tile.text}</p>
            </Reveal>
          ))}
